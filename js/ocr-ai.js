        ...
        var _groqProxyUrl = (window.SUPERZOSHYT_CONFIG && window.SUPERZOSHYT_CONFIG.GROQ_PROXY_URL) || 'https://api.groq.com/openai/v1/chat/completions';

        async function recognizeImageText(data, btn) {
            var _userKey = window.getGroqUserKey ? window.getGroqUserKey() : '';
            ...
            try {
                ...
                if (_userKey) _headers['Authorization'] = 'Bearer ' + _userKey;

                const response = await GroqQueue.enqueue(_groqProxyUrl, {
                    method: 'POST',
                    headers: _headers,
                    body: JSON.stringify({
